...
## 作業時注意メモ
  ...
  本年目標(%)
  本年実績(%)\
  に関しては､スプレッドシートの出力側を15.5%形式に少数第一位までで四捨五入して比較してください｡
  3.下記のように表記揺れがあるのでそれを考慮してください｡
    3-1. main_category表記ゆれ: PDF 売総利益率 vs SS 売上総利益率
    3-2. その他損益の構造: PDFはその他損益配下、SSは独立した行
    3-3. 括弧の全角/半角: PDF (リベート) vs SS （リベート）
  4.SSの支店は1列目で判断してください｡
  5.SSデータはmain_categoryが最初の行にのみあり、続く行はNaNで継承される構造です。forward-fillで対応してください｡
  6.PDF側も区分(大)(main_categoryにあたる項目)が空の行があります。forward-fillが必要です。また数値のカンマと%記号も処理が必要です。
  7.pdfの値が間違っている場合もないわけではないので､その場合は2509損益長崎.pdf形式の{yy}mm損益{
  支店}.pdfデータを参照してください｡累計の項目もあるので注意して､単月の値を参照するようにしてください
  8.売上総利益率を比較する際には､%数値に合わせたうえで(ex. 0.154→15.4)､少数第一位で四捨五入を行ってください(ex 15.4→15)
...
レポート作成月に応じて参照するフォルダが異なるため、source_folderを持つことで参照先を指定できるようにしています。
