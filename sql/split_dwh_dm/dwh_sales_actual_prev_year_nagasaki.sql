/*
============================================================
DWH: 売上高・粗利実績(前年実績) - 長崎支店
============================================================
目的: 前年同月の売上高と粗利実績を組織・部門別に集計(長崎支店)
...
【不明点】
1. 営業所コード061, 065, 066がsales_target_and_achievementsに存在するか要確認
2. division_codeのマッピングが正しいか要確認(特にサッシ工事)
============================================================
*/

CREATE OR REPLACE TABLE `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_prev_year_nagasaki` AS
SELECT
  ...
  CASE
    ...
    WHEN branch_code IN (065, 066) AND division_code = 20 THEN 'サッシ工事'  -- 要確認: サッシ工事のdivision_code
