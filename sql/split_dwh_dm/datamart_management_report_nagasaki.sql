...
データソース:
  - DWHテーブル（長崎支店用: dwh_sales_actual_prev_year_nagasaki, operating_expenses_nagasaki など）
...
sales_actual AS (
  SELECT
    ...
    sales_amount,
    gross_profit_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_nagasaki`
),
...
sales_actual_prev_year AS (
  SELECT
    ...
    sales_amount,
    gross_profit_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_prev_year_nagasaki`
),
...
sales_target AS (
  SELECT
    ...
    detail_category,
    target_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_target_nagasaki`
),
...
operating_expenses AS (
  ...
  FROM `data-platform-prod-475201.corporate_data_dwh.operating_expenses_nagasaki`
),
...
non_operating_income AS (
  ...
  FROM `data-platform-prod-475201.corporate_data_dwh.non_operating_income_nagasaki`
),
...
miscellaneous_loss AS (
  ...
  FROM `data-platform-prod-475201.corporate_data_dwh.miscellaneous_loss_nagasaki`
),
...
recurring_profit_target AS (
  SELECT
    ...
    detail_category,
    target_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.dwh_recurring_profit_target_nagasaki`
),
...
operating_expenses_target AS (
  SELECT
    ...
    detail_category,
    target_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.operating_expenses_target_nagasaki`
),
...
operating_income_target AS (
  SELECT
    ...
    detail_category,
    target_amount
  FROM `data-platform-prod-475201.corporate_data_dwh.operating_income_target_nagasaki`
),

-- ============================================================
-- 全組織×カテゴリ×月の組み合わせを生成
-- 売上関連テーブルと目標テーブルから組み合わせを生成
-- 集計レベル（XX計）は除外して、個人/部門レベルのみを対象とする
-- ============================================================
all_combinations AS (
  SELECT DISTINCT year_month, organization, detail_category
  FROM (
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_nagasaki`
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_prev_year_nagasaki`
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_target_nagasaki`
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.dwh_recurring_profit_target_nagasaki`
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.operating_expenses_target_nagasaki`
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.operating_income_target_nagasaki`
  )
...
cumulative_recurring_profit AS (
  ...
  monthly_profit AS (
    ...
    FROM `data-platform-prod-475201.corporate_data_dwh.dwh_sales_actual_nagasaki` sa
    LEFT JOIN `data-platform-prod-475201.corporate_data_dwh.operating_expenses_nagasaki` oe
      ON sa.detail_category = oe.detail_category
      AND sa.year_month = oe.year_month
    LEFT JOIN `data-platform-prod-475201.corporate_data_dwh.non_operating_income_nagasaki` ni
    ...
    LEFT JOIN `data-platform-prod-475201.corporate_data_dwh.miscellaneous_loss_nagasaki` ml
