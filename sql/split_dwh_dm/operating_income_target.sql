...
fukuoka_target AS (
  ...
  UNION ALL
  SELECT period, '福岡支店', 'GSセンター', 'GSセンター', gs_center FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '福北センター', '福北センター', fukuhoku_center FROM fukuoka_profit_plan WHERE item = '営業利益'
)
