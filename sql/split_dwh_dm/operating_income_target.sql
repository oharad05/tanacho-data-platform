...
nagasaki_target AS (
  ...
  UNION ALL
  SELECT period, '長崎支店', '工事営業部', '工事営業部計', construction_sales_department_total * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '工事営業部', 'ガラス工事', glass_construction_dept * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '工事営業部', 'ビルサッシ', building_sash * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', '硝子建材営業部計', glass_building_material_sales_department_total * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', '硝子工事', glass_construction * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', 'サッシ工事', sash_construction * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', '硝子販売', glass_sales * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', 'サッシ販売', sash_sales * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '長崎支店', '硝子建材営業部', '完成品', finished_products * 1000 FROM nagasaki_profit_plan WHERE item = '営業利益'
),
...
fukuoka_target AS (
  ...
  UNION ALL
  SELECT period, '福岡支店', '工事部', '工事部計', construction_department_total * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '工事部', '硝子工事', glass_construction * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '工事部', 'ビルサッシ', building_sash * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '工事部', '内装工事', interior_construction * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', '硝子樹脂計', glass_resin_total * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', '硝子', glass * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', '建材', building_materials * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', '樹脂', resin * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', 'GSセンター', 'GSセンター', gs_center * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
  UNION ALL
  SELECT period, '福岡支店', '福北センター', '福北センター', fukuhoku_center * 1000 FROM fukuoka_profit_plan WHERE item = '営業利益'
)
