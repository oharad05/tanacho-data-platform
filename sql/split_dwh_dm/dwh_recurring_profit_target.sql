...
tokyo_target AS (
  ...
  UNION ALL
  SELECT period, '東京支店', '工事営業部', '佐々木（大成・鹿島他）', company_sasaki FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '工事営業部', '浅井（清水他）', company_asai FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  ...
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', '硝子建材営業部計', glass_building_material_sales_department FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', '硝子工事', glass_construction FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', 'ビルサッシ', building_sash FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', '硝子販売', glass_sales FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', 'サッシ販売', sash_sales FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', 'サッシ完成品', sash_finished_products FROM tokyo_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '東京支店', '硝子建材営業部', 'その他', others FROM tokyo_profit_plan WHERE item = '経常利益'
),
...
fukuoka_target AS (
  ...
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', 'GSセンター', gs_center FROM fukuoka_profit_plan WHERE item = '経常利益'
  UNION ALL
  SELECT period, '福岡支店', '硝子樹脂部', '福北センター', fukuhoku_center FROM fukuoka_profit_plan WHERE item = '経常利益'
)
