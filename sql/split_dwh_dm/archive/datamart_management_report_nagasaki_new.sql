...
all_combinations AS (
  ...
  WHERE branch = '長崎支店'
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.dwh_recurring_profit_target`
  WHERE branch = '長崎支店'
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.operating_expenses_target`
  WHERE branch = '長崎支店'
    UNION DISTINCT
    SELECT year_month, organization, detail_category FROM `data-platform-prod-475201.corporate_data_dwh.operating_income_target`
  WHERE branch = '長崎支店'
  )
