...
注意:
  ...
  - dwh_sales_actual_nagasakiとJOINしやすいように同じスキーマに統一
============================================================
*/

CREATE OR REPLACE TABLE `data-platform-prod-475201.corporate_data_dwh.dwh_recurring_profit_target_nagasaki` AS
WITH profit_plan AS (
  SELECT
    period,
    item,
...
SELECT period, '工事営業部', 'ガラス工事', glass_construction_dept FROM profit_plan WHERE item = '経常利益'
UNION ALL
SELECT period, '工事営業部', 'ビルサッシ', building_sash FROM profit_plan WHERE item = '経常利益'
...
UNION ALL
SELECT period, '硝子建材営業部', 'サッシ工事', sash_construction FROM profit_plan WHERE item = '経常利益'
UNION ALL
...
UNION ALL
SELECT period, '硝子建材営業部', '完成品', finished_products FROM profit_plan WHERE item = '経常利益';
