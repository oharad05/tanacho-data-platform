...
echo "[7/15] 営業外費用（長崎支店）を更新中..."
bq query \
  --project_id="${PROJECT_ID}" \
  --use_legacy_sql=false \
  < "${SQL_DIR}/non_operating_expenses_nagasaki.sql"

echo "[8/15] 営業外費用（福岡支店）を更新中..."
bq query \
  --project_id="${PROJECT_ID}" \
  --use_legacy_sql=false \
